...
本仓库是 **omni-auth SDK 的仓库**，不是网页应用仓库：

- **唯一产品 = `packages/omni-auth`**（npm 包 `omni-auth`，经 `.github/workflows/publish.yml` 发布）。
- `apps/demo` 仅是开发/演示宿主（Next.js），用于手动测试 SDK 行为，**不是产品、不参与发布**。
- 认证领域逻辑只允许存在于 `packages/omni-auth`；demo 只消费 SDK 公开 API / HTTP 接口（`/api/auth/*`），不存在第二套认证实现。
- 新增功能落在 `packages/omni-auth`（含 vitest 单测）；demo 的改动只服务于验证 SDK 行为。
- 旧约定失效：仓库根目录已无 `src/` —— 宿主代码一律在 `apps/demo/src/`，SDK 代码一律在 `packages/omni-auth/src/`。
...
- **自动建表/迁移（包内单一实现）**：表结构由 omni-auth 包 `schema.ts` 单一管理；
  ...
  然后重新 build 包。`apps/demo` 不再持有任何宿主侧 schema 同步实现
